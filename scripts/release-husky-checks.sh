...
RELEASE_BRANCH_PREFIX="release/husky-checks/${RELEASE_DATE}"

usage() {
  cat <<'USAGE'
Usage:
  pnpm release:husky-checks
...
if [[ "${SKIP_PR:-0}" != "1" ]] && command -v gh >/dev/null 2>&1; then
  gh pr create \
    --base main \
    --head "${RELEASE_BRANCH}" \
    --title "Release agent-j-husky-checks" \
    --body "Release request for agent-j-husky-checks. GitHub Actions will add the generated version and changelog commit."
else

...
RELEASE_BRANCH_PREFIX="release/style/${RELEASE_DATE}"
...
if [[ "${SKIP_PR:-0}" != "1" ]] && command -v gh >/dev/null 2>&1; then
  gh pr create \
    --base main \
    --head "${RELEASE_BRANCH}" \
    --title "Release agent-j-style" \
    --body "Release request for agent-j-style. GitHub Actions will add the generated version and changelog commit."
else

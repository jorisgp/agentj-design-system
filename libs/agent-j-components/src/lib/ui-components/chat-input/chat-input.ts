...
export class ChatInputComponent implements OnDestroy {
  ...
  private applyAutoResize(textarea: HTMLTextAreaElement): void {
    ...
    const rowCountInSideLayout = this.isMultiline()
      ...
      : this.getRowCountFromScrollHeight(
  ...
  private getSideLayoutTextareaWidth(textarea: HTMLTextAreaElement): number {
    const body = textarea.closest(
      '.agentjds-chat-input__body',
    ) as HTMLElement | null;
    if (!body) {
      return textarea.clientWidth;
    }

    const attach = body.querySelector(
      '.agentjds-chat-input__attach',
    ) as HTMLElement | null;
    const actions = body.querySelector(
      '.agentjds-chat-input__actions',
    ) as HTMLElement | null;
  ...
  private getRowCountFromScrollHeight(
    ...
    return Math.max(
      1,
      Math.ceil((scrollHeight - verticalPadding) / lineHeight),
    );
  }
  ...
  private getRowCountForWidth(
    ...
    if (!measurer) {
      return this.getRowCountFromScrollHeight(
        textarea.scrollHeight,
        lineHeight,
        verticalPadding,
      );
